...
FS = 11025; %% all files expected to be 11025 Hz
%%
...
% Pavan - Modified code
% filepath = 'D:\UC Boulder\Courses\Search Engines and Analysis of High Dimensional datasets\Project\ecen5322\Volumes\project\tracks\';
% [status, attr] = fileattrib(filepath);
% if (status == 1)
%     % Reading the UserRead member of the attributes structure to check read
%     % access to the input path given
%     if (attr.UserRead ~= 1)
%         error 'cannot read from input file (does it exist?)'
%     end
% end
...
%         mydisp(logfile,'checking if all files are readable 22kHz wav mono ...')
        mydisp(logfile,'checking if all files are readable 11kHz wav mono ...')
...
%         c.hop_size = 512;
        c.seg_size = 256; %% 23ms if c.fs == 11025Hz
        c.hop_size = 128;
...
%         end
        ...
        audinfo = audioinfo(data.filenames{i_files});
        if audinfo.TotalSamples > FS*120,
            x0 = ceil(audinfo.TotalSamples/2-FS*60);
            x1 = floor(audinfo.TotalSamples/2+FS*60);
        else %% use what ever is available
            x0 = 1;
            x1 = audinfo.TotalSamples;
        end

        wav = audioread(data.filenames{i_files},[x0 x1]);
        wav = wav * (10^(96/20));
